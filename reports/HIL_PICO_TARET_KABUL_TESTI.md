...
Durum: gerçek Windows/Pico/kamera oturumunda yürütülüyor. HIL-01, yön hareketi,
boş-hazne tetik çekme/bırakma ve fiziksel E-Stop kesmesi PASS; kalan maddeler
aşağıdaki prosedürle tamamlanacaktır.

## 2026-07-30 uzak HIL ara sonuçları

- HIL-01 bağlantı/preflight: PASS — COM8, `PING/STAT/ARM,1`, gerçek USB taret kamerası `camera-index:2`.
- Hareket yön kalibrasyonu: PASS — Gateway semantik `(pan,tilt)` -> Pico raw `(-tilt,pan)`; sağ-yukarı fiziksel gözlemle doğrulandı.
- Boş-hazne tetik testi: PASS — `LZR,1/OK,LASER_1`, ardından bağımsız Gateway zamanlayıcısıyla `LZR,0/OK,LASER_0`; fiziksel çekme ve bırakma gözlendi.
- HIL-02/HIL-04 E-Stop fiziksel kesme: PASS (operatör saha teyidi) — fiziksel enerji kesmesi ekipçe daha önce test edildi. Bu uzak run yeni video veya gecikme ölçümü üretmedi.
- HIL-03 bağlantı/heartbeat kaybı: PASS — hareket sırasında Pico USB ayrıldı, taret fiziksel durdu, host FAULT/ACK timeout üretti, queue sıfır kaldı; tekrar bağlantıda eski hareket/atış replay edilmeden yeni preflight ve yeni yukarı hareket başarılı oldu.
- HIL-19 kamera hot-unplug/recovery: PASS — ilk native DirectShow crash'i izole child worker, PnP VID/PID doğrulaması, siyah-kare reddi ve stall watchdog ile giderildi; çıkarılmış cihaz `CAMERA_UNAVAILABLE`, tekrar takılan gerçek `camera-index:2` ise `REAL_USB_CAMERA_LIVE` oldu.
- Ayrıntılı seri ve cihaz kaydı: `reports/remote_windows_20260730/README.md`.
...
### 2026-07-30 uygulanan HIL-19 kamera/Gateway kabul sonucu

- Harici taret kamerası: `HD USB Camera`, `camera-index:2`, sabit kimlik
  `VID_32E4:PID_9230`; dahili laptop kamerası kullanılmadı.
- Hot-unplug sırasında backend ayakta kaldı; çıkarılmış cihazın yanlış DirectShow
  kaynağıyla yer değiştirmesi sabit PnP kimliği kontrolüyle reddedildi.
- Yeniden takma + tarama + preview ile kod/environment değiştirmeden
  `REAL_USB_CAMERA_LIVE` geri geldi.
- Kamera stale iken LIVE_TEST preflight `CAMERA_STALE` verdi; hareket ve tetik
  çağrıları `command_sent=false` döndü ve seri kayıtta `SPD,*`/`LZR,1` oluşmadı.
- Kamera geri geldiğinde preflight aynı kullanıcı akışıyla READY oldu; kamera
  gate'i `CAMERA_FRESH` gösterdi.
- Windows PnP denetimi kare ingestion yolundan ayrıldı. Düzeltme sonrası 60
  örnekli ölçümde `last_frame_age_ms` 0–34 ms kaldı; önceki periyodik
  500–650 ms yalancı stale sıçramaları ortadan kalktı.
- Sonuç: **PASS** — HIL-19 madde 1–3 içindeki kamera sahipliği, hot-unplug,
  NO_FIRE/NO_MOTION ve recovery koşulları backend/Gateway düzeyinde karşılandı.
  Kalan ekran çözünürlüğü, klavye erişilebilirliği ve 3B yön görsel kanıtları
  ayrı UI saha kaydı olarak uygulanacaktır.

### 2026-07-30 gerçek kamera + GPU inference ara kabulü

- Taret kamerası, önizleme ve YOLO tek izole capture worker üzerinden birlikte
  çalıştı; inference yolu Windows DirectShow cihazını ikinci kez açmadı.
- Test modeli `balloon_dost_dusman_yolo2.pt` CUDA üzerinde uygulama içinden
  yüklendi. İlk yükleme 5,297 s; steady-state 14 örnekte inference p50
  11,749 ms, p95 15,954 ms.
- Inference sırasında kamera fresh kaldı; sonrasında CommandGateway LIVE_TEST
  preflight tüm kapılarla READY oldu.
- Sonuç: **PASS (entegrasyon/CUDA smoke)**. `conf=0.01` normal oda sahnesinde
  çok sayıda false positive ürettiğinden bu model/eşik için production precision
  kabulü verilmedi; gerçek 14 cm balon saha serisi ayrıca yapılacaktır.

### 2026-07-30 yakın mesafe gerçek kırmızı balon A/B sonucu

- Gerçek balon ham modelde `dusman`, confidence `0.9521` olarak doğrulandı;
  görsel bbox balon sınırlarıyla örtüştü.
- `conf=0.05` hedef-yok sahnede dolap üstündeki küçük kırmızı uçak dekoru en
  fazla `0.137` ile yanlış aday oldu.
- Geçici laboratuvar eşiği `0.15` yapıldı. Hedef-yok 80/80 frame sıfır detection.
- Balon tekrar gösterildiğinde 80/80 frame detection, sıfır miss; confidence
  `0.934–0.949`, ortalama `0.94435`; tamamı CUDA/real_capture.
- Sonuç: **PASS (yakın mesafe A/B)**. Bu değer 15 m nihai eşik değildir; saha
  mesafe serisi ve farklı ışık koşulları tamamlanmadan competition threshold
  dondurulmayacaktır.

### 2026-07-30 profil yenileme + kokpit kamera kontrolü

- Setup hard-refresh artık kayıtlı kamera/model/Pico bağlarını silmiyor; yalnız
  fiziksel preflight ve arm yeniden isteniyor. Windows runtime üzerinde profil
  koruma testi PASS.
- Headless 1920×1080 kokpit görüntüsünde harici `camera-index:2` gerçek akışı
  görünür ve kamera API'si error olmadan çalışıyor.
- Arkadaş geldiğinde yapılacaklar: normal Chrome/GPU üzerinde 3B modelin görünür
  olduğunu doğrula; Pico handshake'i yenile; kısa sağ-yukarı hareket sırasında
  fiziksel taret ile dijital ikiz pan/tilt yönünü aynı kayıtta karşılaştır.
- Normal Chrome/GPU saha tekrarında 800 ms sağ-yukarı ve 1200 ms sol-aşağı
  komutları uygulandı. Fiziksel taret ve 3B ikiz her iki eksende aynı yöne gitti;
  sabit alt gövde/ayaklar hareket grubundan ayrılmış kaldı.
- CAD pitch eksen işareti Gateway tilt semantiğine göre render katmanında
  terslendi. Gateway koordinatı, Pico komutu ve UI'daki fiziksel TILT değeri
  değiştirilmedi.
- Her iki testte Pico `OK,SPD`, `OK,DRIVER_ENABLED`, `OK,STOP` ve
  `OK,DRIVER_DISABLED` döndürdü. Sonuç: **PASS — HIL-19 madde 4 yön eşleşmesi.**
- Sonraki saha adımı ateşsiz gerçek balon takibidir: harici kamera + CUDA YOLO
  hedef seçimi, CommandGateway tracking motion ve hedef kaybında otomatik STOP.

### 2026-07-30 gerçek balon fiziksel takip sonucu

- Gerçek kırmızı balon bbox üzerinden seçildi ve tracking loop fiziksel
  CommandGateway hareketi üretti.
- İlk denemede X doğru, Y ters gözlendi. Tracking güvenli durduruldu ve yalnız
  kapalı çevrim Y PID işareti `invert_y=true` yapıldı; manuel/3B yön eşlemesine
  dokunulmadı.
- İkinci denemede fiziksel taret iki eksende balona doğru takip etti. Operatör
  sonucu “oldu çalışıyor” olarak doğruladı.
- 1270 toplam karede 1212 hedefli ve 58 hedef-kayıp kare işlendi. Kabul sonunda
  tracking STOPPED, hız `0/0` ve ayrıca Gateway safe-stop gönderildi.
- Sonuç: **PASS — iki eksenli gerçek balon takip yönü.** Sonraki zorunlu test,
  hedef kadrajdan çıkarıldığında otomatik STOP ve hedef geri geldiğinde yeniden
  yakalama davranışıdır.

### 2026-07-30 hedef kaybı ve yeniden yakalama sonucu

- İlk ölçümde hedef yokken tracking hızı `0/0` olsa da driver açık kaldı. Kod
  düzeltmesiyle SEARCHING geçişi bir kez `STP + DRV,0` üretir hale getirildi.
- Hedef geri geldiğinde kod/env/preflight değişmeden `DRV,1` ve yeni `SPD`
  komutları üretildi; böylece yeniden yakalama yolu doğrulandı.
- Eski komut NACK'inin seri FAULT durumunu kalıcı bırakması giderildi. Başarılı
  `PING/PONG`, görünür preflight sırasında bağlantı recovery kanıtı olarak FAULT
  durumunu temizliyor; NACK yine logda ve ilgili komut reddinde kalıyor.
- LIVE_TEST takip sırasında hedef merkezlenince otomatik FIRE'ın hareket testini
  kesmesi giderildi. LIVE_TEST/VIDEO_DEMO gerçek manuel FIRE'ı korur; otomatik
  fiziksel FIRE COMPETITION A2/A3'e aittir. Bu takip penceresinde `LZR,1=0`.
- Operatör son tekrarında fiziksel taretin balonu iki eksende kesintisiz takip
  ettiğini doğruladı. Balon çıkarıldığında son `SPD` → `STP` gecikmeleri yaklaşık
  `65,5 ms` ve kısa yeniden algılama sonrasında `81,9 ms`; her iki olayda hemen
  `DRV,0` ACK alındı.
- Final: balloon_count `0`, tracking STOPPED, driver disabled ve ek Gateway
  safe-stop kabul edildi. Sonuç: **PASS — hedef kaybı NO_MOTION + recovery.**

### 2026-07-30 takip profili ve dijital ikiz balon ölçeği

- Operatör karşılaştırmasında `field_baseline_pd` (Saha Referansı) kazandı ve
  varsayılan/aktif takip ayarı olarak seçildi.
- Windows HIL balonu `160 mm`, yarışma varsayılan balonu `140 mm` olarak ayrıldı.
- Bbox → mesafe hesabı iki kamera ekseninin gerçek FOV/odak değerlerini kullanır.
- Dijital ikizde parkur derinlik sıkıştırması artık fiziksel balon çapını
  küçültmez; gerçek ölçekli çekirdek ve ayrı seçim halkası kullanılır.
- Yazılımsal test/build: **PASS**. Kalan kabul: gerçek 16 cm balon kadrajdayken
  yakın/uzak hareketinde görsel ölçeğin operatör tarafından doğrulanması.
- NVIDIA Broadcast sanal kamera probunun ana backend prosesini düşürmesi ayrıca
  giderildi: Windows DirectShow indeksleri artık crash-izole alt proseslerde
  deneniyor. Yeniden başlatma sonrası harici USB kamera, CUDA YOLO, Pico ve
  LIVE_TEST preflight `READY` doğrulandı.
