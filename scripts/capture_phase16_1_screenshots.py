...
OUT = ROOT / "reports" / "screenshots" / "phase16_1_model_semantic_polish"
...
def render_json_page(session_id: str, title: str, payload: dict) -> None:
    ...
    <html>
      ...
      <body><main><div class="badge">MODEL SEMANTICS VERIFIED</div><h1>{html.escape(title)}</h1><pre>{html.escape(body)}</pre></main></body>
    </html>
    """
...
def prepare_backend_state() -> dict:
    ...
    export = backend_request("POST", "/api/reports/generate-ktr-summary", {"notes": "phase16.1 model semantic polish evidence"})
...
def main() -> None:
    ...
    active = backend_request("GET", "/api/models/active")
    ...
    try:
        ...
        screenshot(session_id, "01_models_fixture_not_production.png")
        scroll_to_heading(session_id, "Active Model")
        screenshot(session_id, "02_active_model_semantic_fields.png")
        scroll_to_heading(session_id, "Class Mapping Review")
        screenshot(session_id, "03_class_mapping_clean_roles.png")

        navigate(session_id, "/vision")
        scroll_to_heading(session_id, "YOLO Runtime")
        screenshot(session_id, "04_vision_adapter_consistency.png")

        navigate(session_id, "/")
        screenshot(session_id, "05_dashboard_release_vs_competition_blockers.png")

        navigate(session_id, "/first-run")
        screenshot(session_id, "06_first_run_model_semantics.png")
        ...
        time.sleep(0.5)
        screenshot(session_id, "07_self_test_model_warnings.png")

        render_json_page(session_id, "Reports Model Semantic Export", {"export": export, "active_model": active})
        screenshot(session_id, "08_reports_model_semantic_export.png")

        navigate(session_id, "/interfaces")
        scroll_to_heading(session_id, "KTR-ready 4.3 Preview")
        screenshot(session_id, "09_ktr_model_interface_semantic_text.png")
        ...
        time.sleep(0.5)
        screenshot(session_id, "10_logs_model_semantic_events.png")
